...
esp_err_t bsp_display_new(const bsp_display_config_t *config, esp_lcd_panel_handle_t *ret_panel, esp_lcd_panel_io_handle_t *ret_io)
{
    ...
    const spi_bus_config_t buscfg = ICNA3306_PANEL_BUS_QSPI_CONFIG(
    ...
    ESP_LOGI(TAG, "Install LCD driver of icna3306");
    const icna3306_vendor_config_t vendor_config = {
    ...
    ESP_ERROR_CHECK(esp_lcd_new_panel_icna3306(*ret_io, &panel_config, ret_panel));

    esp_lcd_panel_reset(*ret_panel);
    esp_lcd_panel_init(*ret_panel);
    esp_lcd_panel_set_gap(*ret_panel, EXAMPLE_LCD_X_GAP, EXAMPLE_LCD_Y_GAP);
    esp_lcd_panel_disp_on_off(*ret_panel, true);
    esp_lcd_panel_icna3306_set_brightness(*ret_panel, 80);
...
esp_err_t bsp_touch_new(const bsp_display_cfg_t *config, esp_lcd_touch_handle_t *ret_touch)
{
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_touch_new_i2c_cst816s(tp_io_handle, &tp_cfg, &tp_handle), TAG, "New cst816s failed");
