...
static const char *TAG = "LVGL_ICNA3306_CST816S_QSPI";
...
void app_main(void)
{
    ESP_LOGI(TAG, "ESP32-S3 + ICNA3306 (QSPI) + CST816S + LVGL9 开始初始化...");

    // 1. QSPI 总线初始化
    spi_bus_config_t bus_config = ICNA3306_PANEL_BUS_QSPI_CONFIG(
    ...
    ESP_ERROR_CHECK(esp_lcd_panel_icna3306_set_brightness(panel_handle, 80));

    ESP_LOGI(TAG, "ICNA3306 QSPI 显示初始化完成 (%dx%d)", EXAMPLE_LCD_H_RES, EXAMPLE_LCD_V_RES);
